...
type CartItemWithNameAndImg struct {
	CartItem
	ItemQuantity int     `db:"total_quantity" json:"total_quantity"` // Quantity from items
...
func (c *CartItemDB) GetCartItemswithimage(cartID uuid.UUID) ([]CartItemWithNameAndImg, error) {
	...
	query, args, err := QB.Select(
		"cart_items.cart_id",
		"cart_items.item_id",
		"cart_items.quantity AS quantity",  // Quantity from cart_items
		"items.quantity AS total_quantity", // Quantity from items
		"items.name",
		"items.discount",
		"items.price",
		fmt.Sprintf("CASE WHEN NULLIF(items.img, '') IS NOT NULL THEN FORMAT('%s/%%s', items.img) ELSE NULL END AS img", Domain)).
		From("cart_items").
