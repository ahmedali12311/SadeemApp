...
func (u *UserDB) GetUsers(filters utils.Filters) (*[]User, error) {

	querySQL, args, err := utils.BuildQuery("users", user_columns, filters)
	if err != nil {
		return nil, err
	}

	users := []User{}
	err = u.db.Select(&users, querySQL, args...)
	if err != nil {
		return nil, err
	}
	return &users, nil
