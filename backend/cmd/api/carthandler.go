...
func (app *application) GetCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	Getvendor, err := app.Model.VendorDB.GetVendor(cartItems.VendorID, true)
	if err != nil {
	...
	utils.SendJSONResponse(w, http.StatusOK, utils.Envelope{"cart": u})
}
func (app *application) CheckoutHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.errorResponse(w, r, http.StatusNotFound, "No items in the cart!")
		return
	}
	...
	for _, item := range cartItems {
		...
		if newQuantity < 0 {
			app.errorResponse(w, r, http.StatusBadRequest, "Nout Enough quantites")
			return
		}
