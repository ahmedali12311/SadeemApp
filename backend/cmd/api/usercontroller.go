...
func (app *application) IndexUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	var itemsSortSafelist = []string{"created_at", "name", "email"}
	filters := utils.Filters{
		Page:         page,
		PageSize:     pageSize,
		Sort:         sortColumn,
		SortSafelist: itemsSortSafelist,
		Search:       search,
	}

	Validator := *validator.New()
	utils.ValidateFilters(&Validator, filters)
	if !Validator.Valid() {
	...
	users, err := app.Model.UserDB.GetUsers(filters)
	if err != nil {
		app.handleRetrievalError(w, r, err)
		return
	}
