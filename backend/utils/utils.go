...
func BuildQuery(table string, columns []string, filters Filters) (string, []interface{}, error) {
	queryBuilder := QB.Select(strings.Join(columns, ",")).From(table)

	if filters.Search != "" {
		searchQuery := ""
		if strings.Contains(filters.Search, "@") {
			searchQuery = "email ILIKE ?"
		} else if strings.Contains(filters.Search, "+") || strings.Contains(filters.Search, "-") {
			searchQuery = "phone_number ILIKE ?"
		} else if _, err := strconv.Atoi(filters.Search); err == nil {
			searchQuery = "id = ?"
		} else {
			searchQuery = "name ILIKE ?"
		}
		if searchQuery == "id = ?" {
			queryBuilder = queryBuilder.Where(searchQuery, filters.Search)
		} else {
			queryBuilder = queryBuilder.Where(searchQuery, "%"+filters.Search+"%")
		}
	}
	...
	if filters.Sort != "" {
		...
		queryBuilder = queryBuilder.OrderBy(filters.Sort + " " + sortDirection)
	}
